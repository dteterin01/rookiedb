...
abstract class BPlusNode {
    ...
    protected abstract Optional<Pair<DataBox, Long>> splitNode();
    ...
    /**
     * Find the smallest entry greater than the target key.
     * The comparison is made using the `DataBox.compareTo()` method.
     *
     * @param scannedDataBoxes the data boxes where the search takes place
     * @param key The index key to look for
     *
     * @return The slot found and flag found element or else
     * */
    protected static Pair<Integer, Boolean> smallestUpperBoundSlotFor(List<DataBox> scannedDataBoxes, DataBox key) {
        if (scannedDataBoxes.isEmpty()) {
            return new Pair<>(0, false);
        }

        int r = scannedDataBoxes.size() - 1;
        int l = 0;

        while (l <= r) {
            int mid = (l + r + 1) >>> 1;
            DataBox midDataBox = scannedDataBoxes.get(mid);

            int compareResult = midDataBox.compareTo(key);
            if (compareResult > 0) {
                r = mid - 1;
