...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        // TODO(proj4_integration): Update the following line
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.NL);
        return new BPlusTreeIterator(root.getLeftmostLeaf(), LeafNode::scanAll);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj4_integration): Update the following line
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.NL);
        return new BPlusTreeIterator(root.getLeftmostLeaf(), new ScanGreaterEqualIteratorFactoryAbstract(key));
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> isNeedToSplitRoot = root.put(key, rid);

        isNeedToSplitRoot.ifPresent(dataBoxLongPair -> splitRoot(dataBoxLongPair.getFirst(), dataBoxLongPair.getSecond()));
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        while (data.hasNext()) {
            ...
            splitInfo.ifPresent(dataBoxLongPair -> splitRoot(dataBoxLongPair.getFirst(), dataBoxLongPair.getSecond()));
        }
    }
    ...
    private void splitRoot(DataBox key, Long child) {
        ...
        InnerNode rootNode = new InnerNode(metadata, bufferManager, dataBoxes, children, lockContext);
        updateRoot(rootNode);
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private final AbstractRecordIdIteratorFactory nextIteratorProducer;

        public BPlusTreeIterator(LeafNode currentNode, AbstractRecordIdIteratorFactory nextIteratorProducer) {
            this.currentNode = currentNode;
            this.nextIteratorProducer = nextIteratorProducer;
            this.iterator = nextIteratorProducer.apply(currentNode);
        ...
        public boolean hasNext() {
            ...
            Optional<LeafNode> scannedNextNode = currentNode.loadNextRightNode();
            if (!scannedNextNode.isPresent()) {
                return false;
            }
            currentNode = scannedNextNode.get();
            iterator = nextIteratorProducer.apply(currentNode);
    ...
    private interface AbstractRecordIdIteratorFactory extends Function<LeafNode, Iterator<RecordId>> {}

    private static class ScanGreaterEqualIteratorFactoryAbstract implements AbstractRecordIdIteratorFactory {

        private final DataBox greaterOrEqualThen;

        private ScanGreaterEqualIteratorFactoryAbstract(DataBox greaterOrEqualThen) {
            this.greaterOrEqualThen = greaterOrEqualThen;
        }

        @Override
        public Iterator<RecordId> apply(LeafNode leafNode) {
            return leafNode.scanGreaterEqual(greaterOrEqualThen);
        }
    }
}
