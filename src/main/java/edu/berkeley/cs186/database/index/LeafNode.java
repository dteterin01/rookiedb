...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        try {
            Pair<Integer, Boolean> idx = smallestUpperBoundSlotFor(getKeys(), key);
            int maxOrder = metadata.getOrder();
            if (idx.getSecond()) {
                throw new BPlusTreeException("Unique");
            }

            putByIdx(idx.getFirst(), key, rid);
    ...
    @Override
    protected Optional<Pair<DataBox, Long>> splitNode() {
        int maxOrder = metadata.getOrder();
        LeafNode splitRightNode = generateLeafNodeForSplitRightNode(maxOrder, rightSibling);
        removeRowsFromPageAfterMaxOrder(maxOrder);

        // update rightSibling of current page
        long pageNum = splitRightNode.getPage().getPageNum();
        rightSibling = Optional.of(pageNum);

        return Optional.of(
                new Pair<>(
                        splitRightNode.getKeys().get(0),
                        pageNum
                )
        );
    }

    private void putByIdx(int idx, DataBox key, RecordId rid) {
        keys.add(idx, key);
        rids.add(idx, rid);
    }

    private LeafNode generateLeafNodeForSplitRightNode(int maxOrder, Optional<Long> nextPage) {
        List<DataBox> dataBoxesToRightPageAfterSplit = this.keys.subList(maxOrder, 2 * maxOrder + 1);
        List<RecordId> recordIdsToRightPageAfterSplit = this.rids.subList(maxOrder, 2 * maxOrder + 1);

        // inside constructor invoke fetch new page
        return new LeafNode(
                metadata, bufferManager,
                dataBoxesToRightPageAfterSplit,
                recordIdsToRightPageAfterSplit,
                nextPage,
                treeContext
        );
    }

    private void removeRowsFromPageAfterMaxOrder(int maxOrder) {
        this.rids = this.rids.subList(0, maxOrder);
        this.keys = this.keys.subList(0, maxOrder);
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (keys.size() < maxRecords && data.hasNext()) {
            Pair<DataBox, RecordId> nextRecords = data.next();
            keys.add(nextRecords.getFirst());
            rids.add(nextRecords.getSecond());
        }
        ...
        if (data.hasNext()) {
            ...
            keys.add(nextRecords.getFirst());
            rids.add(nextRecords.getSecond());
            LeafNode rightSibling = new LeafNode(metadata, bufferManager, keys, rids, this.rightSibling, treeContext);
            this.rightSibling = Optional.of(rightSibling.getPage().getPageNum());
            ret = Optional.of(new Pair<>(nextRecords.getFirst(), this.rightSibling.get()));
        }
    ...
    public void remove(DataBox key) {
        Pair<Integer, Boolean> searchResult = smallestUpperBoundSlotFor(keys, key);
        if (searchResult.getSecond()) {
            int idx = searchResult.getFirst();
            keys.remove(idx);
            rids.remove(idx);
        }
        sync();
    ...
    Optional<LeafNode> loadNextRightNode() {
        return rightSibling.map(pageId ->
            LeafNode.fromBytes(metadata, bufferManager, treeContext, pageId)
        );
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // pre-init keys and rid
        //
        // leaf_n (stored on some page)
        // +-------+-------+-------+-------+
        // | k0:r0 | k1:r1 | k2:r2 |       |
        // +-------+-------+-------+-------+

        long rightSibling = buffer.getLong();
        int sizePairs = buffer.getInt();
        List<DataBox> keys = new ArrayList<>(sizePairs);
        List<RecordId> recordIds = new ArrayList<>(sizePairs);

        for (int i = 0; i < sizePairs; i++) {
            keys.add(i, DataBox.fromBytes(buffer, metadata.getKeySchema()));
            recordIds.add(i, RecordId.fromBytes(buffer));
        }
        ...
        return new LeafNode(
                ...
                keys,
                recordIds,
                rightSibling == -1 ? Optional.empty() : Optional.of(rightSibling),
                treeContext
        );
    }
