...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        Pair<Integer, Boolean> idx = smallestUpperBoundSlotFor(keys, key);
        if (idx.getSecond()) {
            return getChild(idx.getFirst() + 1).get(key);
        }
        return getChild(idx.getFirst()).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        try {
            Pair<Integer, Boolean> nextNode = smallestUpperBoundSlotFor(keys, key);
            int nextNodeValue = nextNode.getFirst();
            if (nextNode.getSecond()) {
                nextNodeValue++;
            }

            return getChild(nextNodeValue)
                    .put(key, rid)
                    .flatMap(
                            dataBoxLongPair -> insert(dataBoxLongPair.getFirst(), dataBoxLongPair.getSecond())
                    );
        } finally {
            sync();
        }
    ...
    protected Optional<Pair<DataBox, Long>> splitNode() {
        ...
        DataBox dataBox = keys.get(maxOrder);
        InnerNode splitRightNode = generateRightInnerNodeLeaf(maxOrder);
    ...
    private Optional<Pair<DataBox, Long>> insert(DataBox key, Long child) {
        int maxOrder = metadata.getOrder();
        Pair<Integer, Boolean> idx = smallestUpperBoundSlotFor(keys, key);
        keys.add(idx.getFirst(), key);
        children.add(idx.getFirst() + 1, child);
    ...
    private void removeRowsFromPageAfterMaxOrder(int maxOrder) {
        this.children = this.children.subList(0, maxOrder + 1);
        this.keys = this.keys.subList(0, maxOrder);
    }

    private InnerNode generateRightInnerNodeLeaf(int maxOrder) {
        int shift = maxOrder + 1;
        List<DataBox> dataBoxesToRightPageAfterSplit = keys.subList(shift, shift + maxOrder);
        List<Long> childrenToRightPageAfterSplit = children.subList(shift, shift + maxOrder + 1);

        // inside constructor invoke fetch new page
        return new InnerNode(
                metadata, bufferManager,
                dataBoxesToRightPageAfterSplit,
                childrenToRightPageAfterSplit,
                treeContext
        );
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (!splitInfo.isPresent()) {
            ...
            DataBox splitDataBox = splitInfo.get().getFirst();
            Long child = splitInfo.get().getSecond();

            Optional<Pair<DataBox, Long>> inserted = insert(splitDataBox, child);
